...
class Locky
{
    ...
    public function baseDelayMs(int $ms): static
    {
    ...
    public function multiplier(float $m): static
    {
        $this->multiplier = max(1.0, $m);

        return $this;
    }

    public function maxDelayMs(?int $ms): static
    {
        $this->maxDelayMs = $ms !== null ? max(1, $ms) : null;
    ...
    public function onFail(Closure $cb): static
    {
        $this->onFail = $cb;
    ...
    public function onRetry(Closure $cb): static
    {
        $this->onRetry = $cb;
        ...
        while (true) {
            ...
            if ($lock->get()) {
                try {
                    ...
                    try {
                        $lock->release();
                    } catch (\Throwable $e) { /* TTL may have expired; ignore */
                    }
                }
            }
